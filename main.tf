...
resource "azurerm_lb_rule" "lb_rule" {
  ...
  frontend_port                  = 443
  backend_port                   = 443
  frontend_ip_configuration_name = "${var.name}-priIP"
  probe_id                       = azurerm_lb_probe.lb_probe.id
  backend_address_pool_ids       = [azurerm_lb_backend_address_pool.backend_address_pool.id]
  depends_on = [ azurerm_network_interface_backend_address_pool_association.backend_association ]
}